...
class Level2(BaseScene):
	def __init__(self, sceneManager):
		...
		self.enemies = [Enemy1(self, self.background.rect.midleft), Enemy2(self, self.background.rect.midright)]
		self.sprites.add(self.enemies)
		self.spawnEnemy = 6000
	...
	def update(self, events):
		...
		if self.lastEnemySpawned + self.spawnEnemy < pygame.time.get_ticks():
			...
			enemyPosibilities = [Enemy1(self, postionElection), Enemy2(self, postionElection)]
